/*Problem Statement:
A linked list is a linear data structure used in computer science and programming for storing and organizing a collection
of elements. A linked list can be visualized as a chain of nodes, where each node points to the next node, creating a
sequence. Create a C program on a doubly linked list that has a header node which maintains additional information of
the list like – Number of nodes. Perform insertion at beginning, at end, at a position in the list, perform deletion at
beginning, at end and of an item from the list, also include a display operation to display the contents of the list. Your
program must be a menu driven program that provides choices to perform various operations. The node should contain
the SRN, Name, branch and semester information of students in a college.*/
